...
<script type="text/javascript">
	function update() {
		...
		if (isStudentName == true && isPhone==true && isCardId==true && isEmail==true && classCount==true) {
			...
			$.ajax({
			type : "post",//请求类型
			url : "student/updateStudent.action",//请求的url
...
<body>
	...
	<div class="page">
		<div class="box mtop">
			...
			<div class="rightbox">
				<h2 class="mbx">修改学员信息</h2>
				<div class="cztable" align="center">
					<input type="hidden" class="id" name="id" value="${studentUpdate.id }">
					<table>
						<tr>
							<td colspan="2" align="center">修改学员信息</td>
						</tr>
						...
						<tr align="center">
							...
							<td><input
								...
								value="${studentUpdate.studentName }">
							</td>
						</tr>
						<tr align="center">
							<td>电话号码:</td>
						...
						<tr align="center">
							...
							<td><select name="gender" class="gender">
									<c:forEach items="${dataMap['SEX'] }" var="sex">
										<option value="${sex.valueId }"
											<c:if test="${sex.valueId == studentUpdate.gender}">selected="selected"</c:if>>${sex.valueName
											}</option>
									</c:forEach>
							</select>
							</td>
						</tr>
						<input type="hidden" class="oldClassId"
							value="${studentUpdate.classId }">
						<tr align="center">
							<td>年级:</td>
							<td><select name="classId" class="classId">
									<c:forEach items="${classInfo }" var="c">
										<option value="${c.id }"
											<c:if test="${c.id == studentUpdate.classId}">selected="selected"</c:if>>${c.className
											}</option>
									</c:forEach>
							</select>
							</td>
						...
						<tr>
							<td colspan="2" align="center"><input
								onKeypress="javascript:if(event.keyCode == 32)event.returnValue = false;"
								type="button" value=" 确  认  修  改  " onclick="update()">
							</td>
						</tr>
					</table>
				</div>
